import { Webinar } from '@/api/entities';
...
export default function Calendar() {
  ...
  const { data: webinars = [], isLoading } = useQuery({
    queryKey: ['webinars'],
    queryFn: () => Webinar.list('-start_time')
  });

  const syncCalendarMutation = useMutation({
    mutationFn: () => invokeFunction('syncGoogleCalendar', { action: 'sync' }),
    onSuccess: () => {
  ...
  const createWebinarMutation = useMutation({
    mutationFn: (data) => invokeFunction('syncGoogleCalendar', {
      action: 'create',
      event: {
        title: data.title,
          ...
          <div className="bg-kg-card rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto scrollbar-custom">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-gray-500 text-[18px] mt-1">Google Meet, Zoom, or any meeting platform link</p>
              </div>
